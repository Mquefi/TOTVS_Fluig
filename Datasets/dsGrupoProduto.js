function createDataset(fields, constraints, sortFields) {
	...
	var mapFields = [
		["GENERAL_ID", "BM_GRUPO", "string"],
		["DOCUMENT_TYPE", "BM_DESC", "string"],
		["SEARCH_FIELD", "SEARCH_FIELD", "string"],
		["BM_FILIAL", "BM_FILIAL", "string"],
		["TENANT_TABLE", "", "string"]
	];
	
	var filter = getFilter(constraints, mapFields,{"where":" BM_FILIAL = ''"});
	
	var output = executeQuery("SELECT * FROM " +
			"(SELECT BM_GRUPO, BM_DESC, rtrim(concat(BM_GRUPO, ' - ', BM_DESC)) SEARCH_FIELD,BM_FILIAL, '"
			+ getTenantTable(constraints)
			+ "' TENANT_TABLE from SBM"
			+ getTenantTable(constraints)
			+ " WHERE D_E_L_E_T_ <> '*') r " + filter.where, filter.params);
