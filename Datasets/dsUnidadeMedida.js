function createDataset(fields, constraints, sortFields) {
	...
	var mapFields = [
		["GENERAL_ID", "AH_UNIMED", "string"],
		["DOCUMENT_TYPE", "AH_DESCPO", "string"],
		["SEARCH_FIELD", "SEARCH_FIELD", "string"],
		["AH_FILIAL", "AH_FILIAL", "string"],
		["TENANT_TABLE", "", "string"]
	];
	
	var filter = getFilter(constraints, mapFields,{"where":" AH_FILIAL = ''"});
	
	var output = executeQuery("SELECT * FROM " +
			"(SELECT AH_UNIMED, AH_DESCPO, rtrim(concat(AH_UNIMED, ' - ', AH_DESCPO)) SEARCH_FIELD,AH_FILIAL, '"
			+ getTenantTable(constraints)
			+ "' TENANT_TABLE from SAH"
			+ getTenantTable(constraints)
...
function getTenantTable(constraints) {
	for (var i = 0; i < constraints.length; i++) {
		var c = constraints[i];
		log.warn(c.fieldName + " getTenantTable[fieldName] ");
		if ("TENANT_TABLE".equalsIgnoreCase(c.fieldName)) {
			log.warn(c.initialValue + " getTenantTable[initialValue] ");
			//if (parseInt(c.initialValue)) {
			//	log.warn("ParseInt");
			//	log.warn(parseInt(c.initialValue));
				return c.initialValue;
			//}else{
				
			//	log.warn("ParseInt [ERROR]");
			//	log.warn(parseInt(c.initialValue));
			//}
		}
	}
