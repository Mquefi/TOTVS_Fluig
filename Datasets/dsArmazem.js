function createDataset(fields, constraints, sortFields) {
	...
	var mapFields = [
		["GENERAL_ID", "NNR_CODIGO", "string"],
		["DOCUMENT_TYPE", "NNR_DESCRI", "string"],
		["SEARCH_FIELD", "SEARCH_FIELD", "string"],
		["NNR_FILIAL", "NNR_FILIAL", "string"],
		["TENANT_TABLE", "", "string"]
	];
	
	var filter = getFilter(constraints, mapFields,{"where":" NNR_FILIAL = '"+getTenantFilial(constraints)+"'"});
	
	var output = executeQuery("SELECT * FROM " +
			"(SELECT NNR_CODIGO, NNR_DESCRI, rtrim(concat(NNR_CODIGO, ' - ', NNR_DESCRI)) SEARCH_FIELD,NNR_FILIAL, '"
			+ getTenantTable(constraints)
			+ "' TENANT_TABLE from NNR"
			+ getTenantTable(constraints)
