function createDataset(fields, constraints, sortFields) {
	log.info("@@@ dsCondPagto ");
	log.dir(constraints);
	
	var mapFields = [
		["GENERAL_ID", "E4_CODIGO", "string"],
		["DOCUMENT_TYPE", "E4_COND", "string"],
		["SEARCH_FIELD", "SEARCH_FIELD", "string"],
		["E4_FILIAL", "E4_FILIAL", "string"],
		["TENANT_TABLE", "", "string"]
	];
	
	var filter = getFilter(constraints, mapFields, {"where": "E4_FILIAL = '"+getTenantFilial(constraints)+"'"});
	
	var output = executeQuery("SELECT * FROM " +
			"(SELECT E4_CODIGO, E4_COND, rtrim(concat(E4_CODIGO, ' - ', E4_COND)) SEARCH_FIELD,E4_FILIAL, '" 
			+ getTenantTable(constraints)
			+ "' TENANT_TABLE from SE4"
			+ getTenantTable(constraints)
