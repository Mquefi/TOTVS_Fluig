...
async function setPedido(pedido) {
  ...
  try {
    ...
    itens.forEach((item) => {
      ...
      document.querySelector("#C7_OBS___" + seq).value = String(item.C7_OBS).replace("\x00", "");
...
function clearTable(tableName) {
  [...document.querySelectorAll(`[tablename="${tableName}"] tbody tr`)]
    .filter((tr) => tr.style.display != "none")
    .forEach((tr) => fnWdkRemoveChild(tr));
}
